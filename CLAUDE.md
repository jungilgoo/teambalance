...
- **`lib/auth.ts`**: 인증 시스템 (현재 모크, Supabase Auth 준비)
- **`lib/mock-data.ts`**: 임시 데이터 및 유틸리티 함수 (레거시)
...
### 인증 시스템 (모크 + Supabase 준비)
현재 `lib/auth.ts`에서 제공:
- `socialLogin(provider)`: 임시 로그인 (1초 지연 시뮬레이션)
- `getAuthState()`: 현재 인증 상태 반환
- `logout()`: 로그아웃
...
### 현재 상태 (2025년 1월)
- **완전한 프로덕션 시스템**: Supabase 기반 실제 데이터베이스 + 모든 핵심 기능 동작
- **전체 플로우 완성**: 팀 생성 → 멤버 초대 → 세션 생성 → 팀 밸런싱 → 경기 진행 → 결과 저장 → 통계 업데이트
- **안정성 확보**: 주요 버그 수정 완료, 사용자 경험 최적화
- **확장 기반 마련**: 소셜 로그인, 실시간 기능 구현을 위한 기술적 토대 완성

### 다음 우선순위 구현 계획

#### 🎯 **Phase 1: 소셜 로그인 활성화 (즉시 가능)**
1. **Supabase Auth 설정**
...
#### 🚀 **Phase 2: 실시간 기능 구현 (고가치)**
1. **Supabase Realtime 설정**
   - `lib/supabase.ts`에 실시간 구독 설정
